...
详情视图快捷键：
- `v`：切换 verbose（更深的、带超时与数量上限的采集：监听地址、Memory/IO/FDs/Threads/Children 等）。
- `e`：显示/隐藏 `Env`（Linux 默认显示）。
- `s`：切换 `Env` secrets 显示（仅在 `Env` 开启时生效；默认脱敏）。
...
`Env`（Linux）默认展示且会自动脱敏（仅显示 key，敏感 value 显示为 `<redacted>`）；可在详情视图用 `e/s` 控制显示与明文。
